...
char out_payload[32]={};
int main() {
    vyid::v2::initialize_csprng(nullptr);
    const size_t iterations=10000000;
    cout<<"---------------- VYID Benchmark with "<<iterations<<" iterations ----------------"<<endl;
    vyid::v2::generate_vyid_char(out,sizeof(out),payload_32,sizeof(payload_32));
    cout<<"v2 : Extracting payload, automatic seed, on the same VYIDv2"<<endl;
    auto start=high_resolution_clock::now();
    for (size_t i=0;i<iterations;++i) {
        vyid::v2::extract_payload(out,sizeof(out),out_payload,sizeof(out_payload));
